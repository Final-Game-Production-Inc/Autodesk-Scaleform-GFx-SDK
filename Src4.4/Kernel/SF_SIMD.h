...
//                  be something that is minimally supported on the platform.
    ...
    // NOTE: arm64 on iOS removed support for vtbl intrinsics. Disabling until a suitable replacement is found.
    #elif defined(SF_CPU_ARM_NEON) && !defined(SF_OS_PSVITA) && !(defined(SF_OS_IPHONE) && defined(__LP64__))
