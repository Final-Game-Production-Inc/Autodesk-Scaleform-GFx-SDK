...
// JWB: Mirror what windows.h does so we don't have to include it
...
{
#if !defined(SF_ENABLE_THREADS) || defined(SF_CPU_X86) || defined(SF_OS_WIN32) || defined(SF_OS_IPHONE) || defined(SF_OS_3DS)
...
#elif defined(SF_OS_WIN32) || defined(SF_OS_WINCE) || defined(SF_OS_XBOX360)
...
#endif
    static T     Exchange_NoSync(volatile T* p, T val);
    static T     ExchangeAdd_NoSync(volatile T* p, T val);
    static bool  CompareAndSet_NoSync(volatile T* p, T c, T val);
...
#elif (defined(SF_OS_PS3))
    // NOTE: PS3 has 64-bit atomic operations, even though it does not use 64-bit pointers.
    typedef uint64_t T;
    typedef T* InterlockTPtr;    
    inline static T     Exchange_NoSync(volatile T* p, T val)            { return cellAtomicStore64((InterlockTPtr)p, val); }
    inline static T     ExchangeAdd_NoSync(volatile T* p, T val)         { return cellAtomicAdd64((InterlockTPtr)p, val); }
    inline static bool  CompareAndSet_NoSync(volatile T* p, T c, T val)  { return cellAtomicCompareAndSwap64((InterlockTPtr)p, val, c) == c; }
#elif !defined(SF_64BIT_POINTERS)
    // Provide a type for no 64-bit atomic operations.
    typedef UInt64 T;
...
#elif defined(SF_OS_WINCE) || defined(SF_OS_XBOX360) 
