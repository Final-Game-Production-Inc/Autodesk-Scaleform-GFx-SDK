...
#elif defined(__i386__) || (defined(SF_OS_WIN32) && !defined(_M_ARM_FP))
#  define SF_CPU_X86
...
#  define SF_CPU_ARM
#elif defined(__LP64__) // iOS: arch=arm64
#  define SF_64BIT_POINTERS
#else
#  define SF_CPU_OTHER
#endif
...
#endif // defined(SF_OS_WIN32) || defined(SF_OS_XBOX360) || defined(SF_OS_WINCE)
...
//#if (defined(SF_OS_WIN32) && !defined(__cplusplus_cli))
...
// Xbox360 specific debugging
    ...
    #define SF_DEBUG_BREAK     do { __android_log_assert("Assert", "GFx", "Assertion failure at %s(%d)", __FILE__, __LINE__); } while(0)
#else
...
// A special class that adds operator== to a pointer. Useful to use with Hash.
template <class T> 
struct PtrCmp
{
    PtrCmp() : P() {}
    ...
    bool operator==(const PtrCmp& p) const 
    {
        return (p.P == P) || (P && p.P && *P == *p.P);
    }
    PtrCmp& operator=(T* p) { P = p; return *this; }

protected:
    T* P;
};

// Not NULL Version of PtrCmp.
template <class T> 
struct PtrCmpNN : public PtrCmp<T>
{
    PtrCmpNN(T& v) : PtrCmp<T>(v) {}
    PtrCmpNN(T* p) : PtrCmp<T>(p) { SF_ASSERT(p); }
    PtrCmpNN(const PtrCmpNN& pp) : PtrCmp<T>(pp) { SF_ASSERT(pp); }

    bool operator==(T* p) const 
    {
        return (p == this->P) || (p && *this->P == *p);
    }
    bool operator==(const PtrCmpNN& p) const 
    {
        return (p.P == this->P) || (*this->P == *p.P);
    }
    PtrCmpNN& operator=(T* p) { SF_ASSERT(p); this->P = p; return *this; }
};

template <class T>
inline bool operator==(T* lp, const PtrCmp<T>& rp)
{
    return rp == lp;
}

template <class T>
inline bool operator==(T* lp, const PtrCmpNN<T>& rp)
{
    return rp == lp;
}
