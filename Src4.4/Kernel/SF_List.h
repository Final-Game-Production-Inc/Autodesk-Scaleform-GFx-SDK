...
Created     :   2008
Authors     :   MaximShemanarev, Sergey Sikorskiy
...
struct ListNode
{
    ...
    bool    IsInList() const
    {
        const T* bad = reinterpret_cast<T*>(~UPInt(0));
        return (pPrev != bad && pNext != bad);
    }
...
public:
    ...
    ValueType* GetFirst() const { return (ValueType*)Root.pNext; }
    ValueType* GetLast () const { return (ValueType*)Root.pPrev; }
    ...
    bool HasOneElem() const { return !IsEmpty() && GetFirst() == GetLast(); }
    // Return true if there are two or more elements in the list.
    bool HasTwoPlusElems() const { return GetFirst() != GetLast(); }

    inline static ValueType* GetPrev(const ValueType* p) { return (ValueType*)p->pPrev; }
    inline static ValueType* GetNext(const ValueType* p) { return (ValueType*)p->pNext; }
