...
public:
    ...
    // Return a hi-res timer value in microseconds (1/1000000 of a sec).
    // To avoid overflow, the first call initializes a "zero" value
    // Future calls return the difference
    static UInt64  SF_STDCALL GetProfileTicks();

    // Zero-based profile timer in seconds. First call initializes 
    // the "zero" value; future calls return the difference.
    ...
    static inline Double RawTicksToSeconds(UInt64 rawTicks)
    {
        return static_cast<Double>(rawTicks) / GetRawFrequency();
    }
...
private:
    ...
    static UInt64 GetRawTicksNoOverride();
    static UInt64 GetRawFreqNoOverride();
    static UInt64 StartRawTicks;
    static UInt64 RawFrequency;
};
