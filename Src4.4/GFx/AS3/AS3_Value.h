...
// #define SF_AS3_ENABLE_STACKWINDOW
...
#define SF_A3_ARG(x) , x
#define SF_A3_CODE(x) x
...
struct ThunkInfo 
{
    typedef void (*TThunkFunc)(const ThunkInfo& ti, VM& vm, const Value& _this, Value& result, unsigned argc, Value* argv);
    ...
    UInt32 GetMinArgNum() const
    {
        return static_cast<UInt32>(MinArgNum_);
    }
    UInt32 GetMaxArgNum() const
    {
        return static_cast<UInt32>(MaxArgNum_);
    }
    UInt32 GetDefArgNum() const
    {
        return static_cast<UInt32>(DefArgNum_);
    }
    ...
    static void EmptyFunc(const ThunkInfo& ti, VM& vm, const Value& _this, Value& result, unsigned argc, Value* argv);
    ASString GetQualifiedName(StringManager& sm, QNameFormat f = qnfWithColons) const;
};

///////////////////////////////////////////////////////////////////////////
struct OpStackLayout
{
    OpStackLayout() 
        : DiscardResult(false)
        , ResultOnStack(false)
        , Closure()
        , NsDataNum()
        // ArgC is not initialized on purpose.
    {
    }
...
public:
    ...
    enum ObjectTag {otObject = 0, otNamespace = 2, ObjectTagMask = 0x02};
...
public:
    ...
    /// Pick value from the stack and pop it.
    void Pick(VSBase& v);
    void Pick(StackWindowBase& v);
    void PickUnsafe(VSBase& v);
    void PickUnsafe(StackWindowBase& v);
...
public:
    ...
    Pickable<Object> GetPickableObject()
    {
        SF_ASSERT(IsObject());
        Object* tmp = value.VS._1.VObj;
        value.VS._1.VObj = NULL;
        SetKind(kUndefined);
        return Pickable<Object>(tmp);
    }
...
public:
    ...
    {
        return (GetKind() == kNamespace ? otNamespace : otObject);
    }
...
public:
    ...
    static bool IsObject(const KindType k) { return k == kObject; }
    ...
    static bool IsClosure(const KindType k) { return k >= kThunkClosure; }
    bool IsClosure() const { return IsClosure(GetKind()); }

    static bool IsCallable(const KindType k) { return IsClosure(k) || IsThunk(k) || IsMethodInd(k) || IsVTableInd(k) || k == kObject; }
    ...
    static bool IsTraitsStrict(const KindType k) { return IsTraits(k); }
...
private:
    static ObjType* GetAddrNoTag(ObjType* pObj)
    {
        return (ObjType*)((Scaleform::UPInt)pObj & ~Value::ObjectTagMask);
    }
    ObjType* GetAddrNoTag() const
    {
        return GetAddrNoTag(pObject);
    }
    static ObjType* GetRawAddr(ObjType* pObj)
    {
        return (ObjType*)((UPInt)GetAddrNoTag(pObj) & ~1);
    }
    ObjType* GetRawAddr() const
    {
        return GetRawAddr(pObject);
    }

    static ObjType* MakeAddr(const Value& v);

    static void AddRef(ObjType* pObj);
    void AddRef() const
    {
        AddRef(pObject);
    }
