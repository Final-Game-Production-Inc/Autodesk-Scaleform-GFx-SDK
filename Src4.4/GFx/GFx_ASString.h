...
{        
    ...
    static const unsigned UNDEFINED_LENGTH = static_cast<unsigned>(-1);
    mutable unsigned LastLength;
    mutable unsigned LastIndexAccessed;
    mutable const char* LastBufferPosAccessed;
    ...
    void ClearCachedAccess() const
    {
        LastLength = UNDEFINED_LENGTH;
        LastIndexAccessed = UNDEFINED_LENGTH;
        LastBufferPosAccessed = NULL;
    }
};
...
{
public:
    explicit ASConstString(ASStringNode *pnode) : ASStringNodeHolder(pnode)
    {
        SF_ASSERT(pNode);
        SF_ASSERT(pNode->pData);
    }
    explicit ASConstString(const ASString& other);
    ASConstString(const ASConstString& other) : ASStringNodeHolder(other)
    {
        SF_ASSERT(pNode);
        SF_ASSERT(pNode->pData);
    }
...
inline
ASConstString::ASConstString(const ASString& other) : ASStringNodeHolder(other.GetNode())
{
    SF_ASSERT(pNode);
}
...
public:
    ...
    // Hash update - unused.
    static UPInt    GetCachedHash(const C& data)                { SF_ASSERT(data); return data->HashFlags; }
...
typedef HashSetUncachedDH<ASStringNode*,
...
public:
    ...
    SPInt GetNextInChain() const { return NextInChain; }
    void SetNextInChain(SPInt v)
    {
        NextInChain = v;
    }

    const C& GetValue() const { return Value; }
    C& GetValue() { return Value; }
    void SetValue(const C& v)
    {
        Value = v;
    }
