...
public:
    ...
    struct ActiveSounds : public NewOverrideBase<StatMV_Other_Mem>
    {
        ...
        int                             ExtraVolume;    // volume for ExtraAudio track (if available)
...
//     virtual DisplayObject*      GetMaskOwner() const;
    ...
    void              SetSoundVolume(int volume, int subvol = 100, int extravol = 100);
    ...
    // Return a sound volume for ExtraAudio track which is saved inside this object.
    int               GetExtraSoundVolume();
    ...
    // Return a calculated sound volume for ExtraAudio track.
    float             GetRealExtraSoundVolume();
