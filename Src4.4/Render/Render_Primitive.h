...
public:    
    ...
    virtual void        GetFillMatrix(HAL* hal, MeshBase *mesh, Matrix2F* matrix, unsigned layer,
        unsigned fillIndex, unsigned meshGenFlags)
    {
        SF_ASSERT(0); SF_UNUSED6(hal, mesh, matrix, layer, fillIndex, meshGenFlags);
    }
...
public:    
    ...
    Mesh(MeshProvider *pdataProvider,
...
public:
    PrimitiveFillManager(MemoryHeap* heap) : pHeap(heap) 
    {
        if (pHeap == 0)
        {
            SF_DEBUG_WARNING(1, "Passing NULL Heap to PrimitiveFillManager. Using GlobalHeap by default.");
...
private:
    ...
    MemoryHeap*     pHeap;      // The heap in which PrimitiveFills are allocated. Generally this is the HAL's heap.
    FillHashSet     FillSet;
...
protected:
    ...
    // directly for text.
    QIPrepareResult  prepare(HAL* hal, void* item,
                             PrimitivePrepareBuffer*, PrimitiveEmitBuffer*,
                             MeshCache* cache, bool waitForCache);
    void             emitToHAL(HAL* hal, void* item,
                               PrimitivePrepareBuffer*, PrimitiveEmitBuffer*);
...
public:
    ComplexMesh(MeshProvider *meshProvider,
...
//    HAL* GetHAL() const { return pFillManager->GetHAL(); }
    ...
    struct FillRecord
    {
        ...
        VertexFormat*       pFormats[2];        // [0] = Single, [1] = Instanced vertex formats.
    ...
    bool            InitFillRecords(HAL*hal, const VertexOutput::Fill* fills, unsigned fillCount,
                                    const Matrix2F& vertexMatrix, UPInt *vbSize, unsigned *vertexCount, 
                                    unsigned *indexCount);
        
private:    
    void updateFillMatrixCache(HAL* hal, const Matrix2F& vertexMatrix);
    bool updateFills(HAL* hal);
