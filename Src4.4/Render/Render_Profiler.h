...
inline bool ProfilerHAL<HALBase>::BeginScene()
{
    ...
    HALBase::GetProfiler().BeginScene(this);
...
{
    HALBase::GetProfiler().SetDrawMode(DrawMode_Fill);
...
{
    HALBase::GetProfiler().SetDrawMode(DrawMode_Mask);
...
inline void ProfilerHAL<HALBase>::DrawProcessedPrimitive(Primitive* pprimitive, PrimitiveBatch* pstart, PrimitiveBatch *pend)
{
    ...
    if (HALBase::GetProfiler().ShouldDrawMaskVisible() && (HALBase::HALState & HALBase::HS_DrawingMask))
    {
...
{
    if (HALBase::GetProfiler().ShouldDrawCachedPrimArea())
    {
...
inline void ProfilerHAL<HALBase>::PushBlendMode(BlendPrimitive* prim)
{
    ...
    {
        typename HALBase::BlendModeStackEntry e = {prim, 0, 0, false, false};
...
{
    DrawModeType previousDrawMode = HALBase::GetProfiler().GetDrawMode();
    HALBase::GetProfiler().SetDrawMode(DrawMode_Clear);
    color = HALBase::GetProfiler().GetClearColor(color);
    HALBase::ClearSolidRectangle(r, color, blend);
    HALBase::GetProfiler().SetDrawMode(previousDrawMode);
}
...
inline bool ProfilerHAL<HALBase>::ShouldRenderFilters(const FilterPrimitive* prim) const
{
    ...
    if (HALBase::GetProfiler().GetProfileMode() != Profile_None)
        return false;
