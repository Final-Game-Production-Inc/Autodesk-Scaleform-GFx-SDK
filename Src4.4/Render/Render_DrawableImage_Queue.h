...
struct DICommand_SourceRect : public DICommand
{
    ...
    virtual bool CalculateDestClippedRect( Rect<SInt32> &dstClippedRect, Point<SInt32> &delta ) const;
};
...
struct DICommand_SourceRectImpl : public DICommand_SourceRect
{
    ...
    virtual void ExecuteHWGetImages( DrawableImage** images, Size<float>* readOffsets, const Rect<SInt32>& destClippedRect) const
    {
        unsigned index = 0;
        if ( GetRequireSourceRead() )
        {
            images[index] = pImage;
            readOffsets[index++] = Size<float>((float)destClippedRect.x1, (float)destClippedRect.y1);
        }
        images[index] = pSource;
        Point<SInt32> negDest = (-DestPoint);
        negDest.ClampMin(0);
        Point<SInt32> srcTopLeft = SourceRect.TopLeft() + negDest;
        srcTopLeft.ClampMin(0);
        readOffsets[index++] = Size<float>((float)srcTopLeft.x, (float)srcTopLeft.y);
    }
...
{
    // Note: the DICommand structures are not necessarily aligned when allocated in command pages. Since the Cxform
    // struct defines its members as aligned, some compilers may vectorize its members automatically, and this could
    // cause an exception when copy constructing a Cxform into a dynamically allocated DICommand_ColorTransform. Instead,
    // store the member data as a byte buffer.
    UByte CxBuffer[sizeof(Cxform)];
    ...
    {
        memcpy(CxBuffer, cxform.M, sizeof(CxBuffer));
    }
