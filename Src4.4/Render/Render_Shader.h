...
public:
    ...
    virtual bool SetStaticShader(typename Shader::ShaderType type, const VertexFormat* pvf) = 0;

    // SetFilterShader stub, for platforms which generate filter shaders dynamically.
    bool SetFilterShader(const Matrix2F& viewMatrix, const Filter* filter, typename Shader::ShaderType type, unsigned pass, unsigned passCount, const VertexFormat* pvf)
    {
        SF_UNUSED4(viewMatrix, filter, pass, passCount);
        return SetStaticShader(type, pvf);
    }
...
public:
    ...
    ShaderType StaticShaderForFill (PrimitiveFill* pfill, unsigned& fillflags, unsigned batchType)
    {
        ...
        if (pfill->GetTextureCount() >= 1 && pfill->GetTexture(0)->GetPlaneCount() > 1)
        {
            // Override these specifically for textures with multiple planes.
            if ( (pfill->GetTexture(0)->GetFormat() >= Image_VideoFormat_Start &&
                  pfill->GetTexture(0)->GetFormat() <= Image_VideoFormat_End) ||
                 pfill->GetTexture(0)->GetFormat() == Image_ETC1A_RGBA_4BPP)
            {
                ...
                switch(pfill->GetTexture(0)->GetFormat())
                {
                    ...
                    case Image_ETC1A_RGBA_4BPP:
                        SF_DEBUG_WARNONCE1(pfill->GetTexture(0)->GetPlaneCount() != 2, 
                            "Image_ETC1A_RGBA_4BPP should have 2 planes, but it has %d", pfill->GetTexture(0)->GetPlaneCount());
                        switch(type)
                        {
                        default:
                        case PrimFill_Texture:
                        case PrimFill_Texture_EAlpha:
                            shader  = ShaderDesc::ST_ATexTG;
                            break;
                        case PrimFill_Texture_VColor:
                        case PrimFill_Texture_VColor_EAlpha:
                            shader  = ShaderDesc::ST_ATexTGVertex;
                            break;
                        case PrimFill_2Texture:
                        case PrimFill_2Texture_EAlpha:
                            shader  = ShaderDesc::ST_ATexTGATexTG;
                            break;
                        case PrimFill_UVTexture:
                            shader  = ShaderDesc::ST_ATexUV;
                            break;
                        }
                        break;

                    default:
                        SF_DEBUG_ASSERT1(0, "Unknown multiple-plane texture format: %d\n", pfill->GetTexture(0)->GetFormat());
                        shader = ShaderDesc::ST_YUV;
                        break;
                }
    ...
    const Shader& SetFill(PrimitiveFill* pfill, unsigned& fillFlags, unsigned batchType, const VertexFormat* pvf, ShaderInterface* psi)
    {
        ShaderType shader = StaticShaderForFill(pfill, fillFlags, batchType);
        psi->SetStaticShader(shader, pvf);
        psi->BeginPrimitive();
    ...
    // This function is extracted, because generated filter shaders generally need to replace the fsize.x/.y inside the shader code at runtime.
    void GenerateBlurFilterParameters( const BlurFilterParams &params, ShaderType type, unsigned pass, float * fsize, const Matrix2F &viewMatrix, float * texscale, unsigned passCount )
    {
        // NOTE: If there are zero passes, do not apply any blur, but, still render the results.
        float blurx = Alg::Max( 1.0f, floorf(TwipsToPixels(params.BlurX * (params.Passes ? 1 : 0))));
        float blury = Alg::Max( 1.0f, floorf(TwipsToPixels(params.BlurY * (params.Passes ? 1 : 0))));

        if ( type == ShaderDesc::ST_Box1Blur || type == ShaderDesc::ST_Box1BlurMul)
        {
            // On even passes, do the X blur, on odd passes, do the Y blur.
            if ( (pass & 1) == 0 )
            {
                fsize[0] = (blurx-1)* 0.5f * viewMatrix.Sx();
                fsize[1] = 0;
                fsize[3] = 1.0f / (floorf(fsize[0]*2.0f + 1.0f));
                texscale[1] = 0;
            }
            else
            {
                fsize[0] = (blury-1)*0.5f * viewMatrix.Sy();
                fsize[1] = 0;
                fsize[3] = 1.0f / (floorf(fsize[0]*2.0f + 1.0f));
                texscale[0] = 0;
            }
        ...
        {
            fsize[0] = 0.0f;
            fsize[1] = (blury-1)*0.5f * viewMatrix.Sy();
            fsize[3] = 1.0f / (floorf(fsize[1]*2.0f + 1.0f));
        }
        else
        {
            fsize[0] = (blurx-1)*0.5f * viewMatrix.Sx();
            fsize[1] = (blury-1)*0.5f * viewMatrix.Sy();
            fsize[3] = 1.0f / (floorf(fsize[0]*2.0f + 1.0f) * floorf(fsize[1]*2.0f + 1.0f));
        }
    ...
    bool SetFilterFill(const Matrix2F& mvp, const Cxform & cx, const Matrix2F& viewMatrix, const Filter* filter, Ptr<RenderTarget> * targets, 
                       unsigned* shaders, unsigned pass, unsigned passCount, const VertexFormat* pvf, 
                       ShaderInterface* psi)
    { 
        if (!psi->SetFilterShader(viewMatrix, filter, (ShaderType)shaders[pass], pass, passCount, pvf))
        {
            return false;
        }
        ...
        if ( filter->GetFilterType() <= Filter_Blur_End )
        {
            ...
            GenerateBlurFilterParameters(params, (ShaderType)shaders[pass], pass, fsize, viewMatrix, texscale, passCount);
            ...
            if ( shadowPass )
            {
                ...
                offset[0] = -TwipsToPixels(params.Offset.x) * viewMatrix.Sx();
                offset[1] = -TwipsToPixels(params.Offset.y) * viewMatrix.Sy();
                ...
                {
                    const GradientFilter* gradientFilter = static_cast<const GradientFilter*>(filter);
        ...
        {
            const DisplacementMapFilter* dmFilter = static_cast<const DisplacementMapFilter*>(filter);
            ...
            float compx[4], compy[4], mapScale[4], scale[4];
            ...
            mapScale[0] = ((float)ptexture->GetSize().Width / pmaptex->GetSize().Width) / viewMatrix.Sx();
            mapScale[1] = ((float)ptexture->GetSize().Height / pmaptex->GetSize().Height) / viewMatrix.Sy();
            mapScale[2] = (dmFilter->MapPoint.x / pmaptex->GetSize().Width);
            mapScale[3] = (dmFilter->MapPoint.y / pmaptex->GetSize().Height);

            scale[0] = (dmFilter->ScaleX / ptexture->GetSize().Width) * viewMatrix.Sx();
            scale[1] = (dmFilter->ScaleY / ptexture->GetSize().Height) * viewMatrix.Sy();

            // NOTE: if the input map is not the same dimensions as its texture, scale it slightly smaller,
            // such that one pixel is cut off the side, otherwise, artifacts may be present if the last pixel
            // is sampled.
            if (dmFilter->DisplacementMap->GetSize() != pmaptex->GetSize())
            {
                scale[2] = ((float)dmFilter->DisplacementMap->GetSize().Width-1) / pmaptex->GetSize().Width;;
                scale[3] = ((float)dmFilter->DisplacementMap->GetSize().Height-1) / pmaptex->GetSize().Height;
            }
            else
            {
                scale[2] = 1.0f;
            ...
            psi->SetUniform(pso, Uniforms::SU_scale, scale, 4);
    ...
    unsigned GetFilterPasses(const Filter* filter, unsigned fillFlags, unsigned* passes ) const
    {
        ...
        if ( filter->GetFilterType() <= Filter_Blur_End )
        {
            ...
            {
                // NOTE: might have zero passes for a non-CaB filter, if the quality is set to zero. This causes
                // the filter to be rendered, but no blurring is applied.
                if (filter->GetFilterType() != Filter_CacheAsBitmap)
                    passCount = Alg::Max<unsigned>(1,params.Passes);
                else
                    passCount = 0;
            }
            ...
            int pass;
            for ( pass = 0; pass < Alg::Max<int>(0,passCount-1); ++pass )
    ...
    // Sets up the filter to be rendered. 
    ...
    bool SetDrawableCxform( Render::Texture** tex, const Matrix2F* texgen, const Size<int> texsize, 
                            ...
                            const VertexFormat* pvf, ShaderInterface* psi, unsigned flags = 0 )
    {
        // NOTE: Assuming that the vertex buffer contains batch indices.
        if (!psi->SetStaticShader(ShaderDesc::ST_BatchTexTGCxform, pvf ))
            return false;
        psi->BeginPrimitive();
        const Shader& pso = psi->GetCurrentShaders();
                              ...
                              ShaderInterface* psi, unsigned flags = 0 )
    {
        ...
        {
            SF_DEBUG_ASSERT(texsize.Area() > 0, "Invalid texsize.");
        ...
        switch(mode)
        {
            ...
            case Blend_Layer:       
                ...
                if (ptextures[Target_Alpha] == 0)
                {
                    ...
                    shaderType = ShaderDesc::ST_BatchTexTGCxformAc;
