...
// The TreeCacheNode tree is built on the first call to HAL::Draw, with its nodes
...
public:    
    ...
    TreeCacheNode(TreeNode* pnode, unsigned flags);
    ...
    virtual void    HandleChanges(HAL* hal, unsigned changeBits) = 0;
    ...
    void            UpdateInsertIntoParent(HAL* hal,
                                           TreeCacheNode* pparent,
                                           TreeCacheNode* pinsert,
                                           const TreeNode::NodeData* nodeData,
                                           UInt16 depth);
    virtual void    UpdateChildSubtree(HAL* hal, const TreeNode::NodeData*, UInt16 depth);
    // Removed node from a cached parent child list, also updating the Primitive.
    // Used by HandleChanges.
    void            RemoveFromParent(HAL* hal);
    // Removes this node by clearing out pRoot in the subtree.
    virtual void    HandleRemoveNode();


    virtual void    UpdateTransform(HAL* hal,
                                    const TreeNode::NodeData* pdata,
                                    const TransformArgs& t, TransformFlags flags) = 0;

    void            updateMaskCache(HAL* hal, const TreeNode::NodeData* data, UInt16 depth,
                                    bool updateSubtree = true);

    void            updateMaskTransform(HAL* hal, TransformArgs& args, TransformFlags flags);
    void            updateEffectChain(HAL* hal, BundleEntryRange* contentChain);

    inline void     addParentToDepthPatternUpdate();

    virtual void    propagateMaskFlag(HAL* hal, unsigned partOfMask);
    virtual void    propagateScale9Flag(HAL* hal, unsigned partOfScale9);
    virtual void    propagateEdgeAA(HAL* hal, EdgeAAMode parentEdgeAA);
    virtual void    propagate3DFlag(HAL* hal, unsigned parent3D);
    ...
    TransformFlags updateCulling(HAL* hal, const TreeNode::NodeData* data, const TransformArgs& t,
                                 RectF* cullRect, TransformFlags flags);

    void updateFilterCache(HAL * hal, const TreeNode::NodeData* data, const TransformArgs &t, 
        TransformFlags flags, RectF* cullRect, const Matrix3F& m3, bool is3d );
    void updateBlendCache(HAL * hal, const TreeNode::NodeData* data, const TransformArgs &t, 
        TransformFlags flags, RectF* cullRect, const Matrix3F& m3, bool is3d );
    ...
    */
    ...
    virtual void    UpdateBundlePattern(HAL* hal, unsigned flags);
    ...
    virtual bool    GetPatternChain(HAL* hal, BundleEntryRange* range, unsigned flags)
    {
        SF_UNUSED2(hal, flags);
    ...
    virtual void    forceUpdateImages(HAL* hal) { SF_UNUSED(hal); }
};
...
public:
    ...
    TreeCacheContainer(TreeNode* pnode, unsigned flags)
    : TreeCacheNode(pnode, flags),
    ...
    // TreeCacheNode API.
    virtual void    HandleChanges(HAL* hal, unsigned changeBits);
    virtual void    HandleRemoveNode();
    virtual void    UpdateChildSubtree(HAL* hal, const TreeNode::NodeData *pdata, UInt16 depth);

    virtual void    UpdateTransform(HAL* hal, const TreeNode::NodeData* pdata,
    ...
    void            BuildChildPattern(HAL* hal, BundleEntryRange* pattern, unsigned flags);
    virtual void    UpdateBundlePattern(HAL* hal, unsigned flags);
    
    virtual bool    GetPatternChain(HAL* hal, BundleEntryRange* range, unsigned flags);
    virtual bool    IsPatternChainValid() const;    

    virtual void    forceUpdateImages(HAL* hal);
};
...
public:
    ...
    TreeCacheRoot(unsigned flags, TreeRoot* pnode)
        : TreeCacheContainer(pnode, flags),
          pUpdateList(0), ViewValid(false), EnableBlendTarget(false), DepthUpdatesChained(false), 
...
protected:
    ...
    virtual void ComputeFinalMatrix(const TransformArgs& t, MatrixPool& mpool, TransformFlags flags);
...
public:
   ...
   // inline DrawableBundle* GetBundle() const;
    ...
    TreeCacheMeshBase(TreeNode* pnode, const SortKey& key, unsigned flags);
    ~TreeCacheMeshBase();
    ...
    virtual MeshBase*   GetMesh(HAL* hal) = 0;    
