...
//#ifdef SF_BUILD_DEBUG
//// Indicates whether or not two angles are equal.
//template <typename T>
//bool SF_EqualAngles(T a, T b, T tol)
//{
//    T TWO_PI = (T)(2*SF_MATH_PI);
//    T diff   = Alg::Abs(a - b);
//    if (diff <= tol)
//        return true;
//    else if (diff + tol < TWO_PI)
//        return false;
//    else if (diff <= tol + TWO_PI)
//        return true;
//
//    // The angles are more than 2*PI apart.
//    int n         = (int)(0.001 + floor(diff/TWO_PI));
//    diff          = diff - (n*diff);
//
//    //SF_ASSERT(0 <= diff);
//    //SF_ASSERT(diff <= period + SF_MATH_EPSILON);
//    if (diff <= tol)
//        return true;
//    else if (diff + tol < TWO_PI)
//        return false;
...
public:
    ...
    // Scaling
    // use basis vectors
    T GetXScale() const                 { return (T)sqrt((T)(M[0][0]*M[0][0]+M[1][0]*M[1][0]+M[2][0]*M[2][0])); }
    T GetYScale() const                 { return (T)sqrt((T)(M[0][1]*M[0][1]+M[1][1]*M[1][1]+M[2][1]*M[2][1])); }
    T GetZScale() const                 { return (T)sqrt((T)(M[0][2]*M[0][2]+M[1][2]*M[1][2]+M[2][2]*M[2][2])); }
    ...
    // Decompose matrix into translation, scale, rotation, skew and perspective components.
...
template<typename T>
inline void Matrix4x4<T>::GetAngles(T *eX, T *eY, T *eZ) const
{
    // Unscale the matrix before extracting term
    Matrix4x4<T> copy(*this);

    T tol = (T)0.001;
    if (tol < Alg::Abs(1 - Alg::Abs(copy.GetDeterminant())))
    {
        // knock out the scale
        copy.SetXScale( (T)1 );
        copy.SetYScale( (T)1 );
...
//    SF_DEBUG_ASSERT(Alg::Abs(dot01) < tol, "Rows 0 and 1 are not orthogonal, but the code assumes they are.\n");
//    SF_DEBUG_ASSERT(Alg::Abs(dot02) < tol, "Rows 0 and 2 are not orthogonal, but the code assumes they are.\n");
//    SF_DEBUG_ASSERT(Alg::Abs(dot12) < tol, "Rows 1 and 2 are not orthogonal, but the code assumes they are.\n");
//#endif
...
inline void Matrix4x4<T>::GetEulerAngles(T *eX, T *eY, T *eZ) const
{
    ...
    // Assuming the angles are in radians.
    if (copy.M[1][0] > (T)0.998f) {                        // singularity at north pole
        if (eY)
    ...
    if (copy.M[1][0] < (T)-0.998f) {                       // singularity at south pole
        if (eY)
            *eY = (T)atan2(copy.M[0][2], copy.M[2][2]);
        if (eZ)
            *eZ = (T)(-SF_MATH_PI/2.0);
...
// Negates the matrix
template<typename T>
inline void Matrix4x4<T>::Negate()
{
...
// This follows the 3D matrix decomposition in the W3 css working group document on
// css-transforms (which is a modification of Thomas' unmatrix algorithm in Graphic Gems II).
// M = Perspective * Translation * Rotation * Shear * Scale.  We extract the transforms from
// left to right.
template <typename T>
inline bool Matrix4x4<T>::Decompose(T (&t)[3], T (&s)[3], T (&k)[3],
                                    T (&p)[4], T (&q)[4], RotationType rotType) const
{
    if (!IsValid())
        return false;

    if (abs(M[3][3]) < FLT_EPSILON)
        return false;
    ...
    // Copy the matrix, so we can modify the elements
    const T* pData = Data();
    Matrix3x4<T> mat(pData, 12);

    // Skip scaling when M[3][3] is approximately 1
    if (FLT_EPSILON < Alg::Abs(1 - M[3][3]))
    {
        // Divide all entries by M[3][3].
        T scaleFactor = 1 / M[3][3];

        for (unsigned int i = 0; i < 3; ++i)
            for (unsigned int j = 0; j < 3; ++j)
                mat.M[i][j] *= scaleFactor;
    }

    // Get the Perspective

    // Extracting the Perspective doesn't change the translation,
    // and we need to remove the translation to find p.  Therefore
    // we get the translation prior to getting the perspective (even
    // though this could be confusing, because it doesn't follow
    // the transform order).
    ...
    // M = 4x4 matrix
    // Scale M by element M[3][3]
    // b = Extract transformed perspective components: first 3 elements of the 4th row of this Matrix
    // If |b| < epsilon
    //     set p = [0, 0, 0] and p[3] = 1
    // Else
    //     T = Translation components: first 3 elements of 4th column
    //     M.Invert(); M.Transpose();
    //     p = M.Transform(b); p[3] = M[3][3] - p.Dot(T) = 1.0 - p.Dot(T);
    //
    // Get the Translation:
    // T = M[0][3], M[1][3], M[2][3]

    Point3<T> b(M[3][0], M[3][1], M[3][2]);
    if (b.DistanceSquared() < SF_MATH_EPSILON)
    {
        p[0] = 0;
        p[1] = 0;
        p[2] = 0;
    ...
    {
        Point3<T> perspective = mat.TransformNormal(b);
        p[0] = perspective.x;
        p[1] = perspective.y;
        p[2] = perspective.z;

        Point3<T> translation(t[0], t[1], t[2]);
        p[3] = 1 - perspective.Dot(translation);
    }

    // Doing it this way re-uses code, at the cost of
    // calculating the 3x3 determinant a second time.

    // The translation has already been captured.
    T dummy[3];
    bool status = mat.Decompose(dummy, s, k, q, rotType);

    return status;
}


// M = Perspective * Translation * Rotation * Shear * Scale.
// Apply the transforms from right to left.
template <typename T>
inline void Matrix4x4<T>::Recompose(const T (&t)[3], const T (&s)[3], const T (&k)[3],
                                    const T (&p)[4], const T (&q)[4], RotationType rotType)
{
    // Use the 3x4 recompose to apply all transforms, except the perspective
    Matrix3x4<T> m;
    m.Recompose(t, s, k, q, rotType);

    // Copy m to M (with the translation)
    M[0][0] = m.M[0][0];
    M[0][1] = m.M[0][1];
    M[0][2] = m.M[0][2];
    M[0][3] = m.M[0][3];
    ...
    // Assumption: Data consistent with Decompose format,
    // so that the recomposed matrix will have M[3][3] == 1.
    // This assumes p[3] = 1 - perspective.Dot(translation),
    // so that after the perspective is applied to the 3x4
    // matrix, the M[3][3] element should be 1.  Rather than
    // attempting to determine the correct value for M[3][3],
    // we set it to 1.
    //
    // TODO: Validate this assumption and handle deviations.
    M[3][0] = 0;
    M[3][1] = 0;
    M[3][2] = 0;
    M[3][3] = 1.0f;
    ...
    // Force M[3][3] to be 1.
    // Verify M[3][3] is approximately 1?
    M[3][3] = 1;
}

template <typename T>
inline void Matrix4x4<T>::Interpolate(const Matrix4x4<T>& a, const Matrix4x4<T>& b, T t)
{
    ...
    T pA[4], pB[4];
    T qA[4], qB[4];
    a.Decompose(tA, sA, kA, pA, qA, RotationType_Quaternion);
    b.Decompose(tB, sB, kB, pB, qB, RotationType_Quaternion);

    // Think about using nlerp instead of slerp.
    Quaternion<T> Q;
    Quaternion<T> q1(qA[0], qA[1], qA[2], qA[3]);
    Quaternion<T> q2(qB[0], qB[1], qB[2], qB[3]);
    Q.slerp(q1, q2, t);
    Q.Normalize();

    pA[3] = Alg::Lerp(pA[3], pB[3], t);
    for (unsigned i = 0; i < 3; ++i)
    {
        sA[i] = Alg::Lerp(sA[i], sB[i], t);
        tA[i] = Alg::Lerp(tA[i], tB[i], t);
        kA[i] = Alg::Lerp(kA[i], kB[i], t);
        pA[i] = Alg::Lerp(pA[i], pB[i], t);
    }
    ...
    Recompose(tA, sA, kA, pA, qA, RotationType_Quaternion);
}
...
template <typename T>
inline void GetRotationMatrix(Matrix4x4<T>& m, const Quaternion<T>& q)
{
