...
struct Interfaces
{
    ...
    Interfaces(TextureManager* pmanager = 0, HAL* hal = 0, ThreadId rtid = 0)
        : pTextureManager(pmanager), pHAL(hal), RenderThreadID(rtid)
    { }
    Interfaces(const Interfaces& other)
        : pTextureManager(other.pTextureManager), pHAL(other.pHAL),
          RenderThreadID(other.RenderThreadID)
    { }
...
// The SingleThreadedCommandQueue can be used in applications that do not separate
// their Render thread and Simulation thread. It executes all commands immediately.
// It assumes that the renderer interfaces can all be queried from the HAL.
