...
class MeshCacheItem : public ListNode<MeshCacheItem>, public FenceResource
{
    friend class MeshCacheListSet;
    friend struct MeshCacheItemUseNode;
public:
    ...
    MeshCacheListType   GetListType() const     { return ListType; }
...
protected:
    ...
    // Memory for the MeshCacheItem is allocated via alloc, so that a variable size array for the Meshes
    // can be created. Placement new and delete are then used to call the constructor/destructor for the item.
    MeshCacheItem(MeshCacheItem::MeshType type, MeshCacheListSet* pcacheList, 
        MeshBaseContent& content, UPInt allocSize, UPInt classSize,
        unsigned vertexCount, unsigned indexCount);
    virtual ~MeshCacheItem();
    static UByte*  alloc(MemoryHeap* pheap, UPInt classSize,MeshBaseContent& content);
    ...
    MeshType                    Type;
    unsigned                    HashKey;        // Hash table data, used for hashing this batch.
    MeshCacheListSet*           pCacheList;    
    MeshCacheListType           ListType;
    MeshBase**                  pMeshes;        // Meshes in this item. NOTE: Meshes start past the end of the buffer.
...
public:
    ...
    bool    EvictPendingFree(AllocAddr& a, bool forceWait=false);
...
public:
    ...
    {
        SF_DEBUG_ASSERT(pmesh->PinCount != 0, "StagingBuffer Attempting to unpin mesh, which is not currently pinned.");
        if (pmesh->PinCount > 0)
        {
            pmesh->PinCount--;
            if (!pmesh->PinCount)
...
public:
    ...
    {
        // The system is capable of converting one GPU buffer to another. Consoles generally always have this capability,
        // because their video memory is directly accessible by the CPU, and thus the conversion can easily be written in CPU code.
        // However, other systems can also support this, if they for example support compute shaders that can do the conversion.
    ...
    virtual bool    PreparePrimitive(HAL* hal, PrimitiveBatch* pbatch,
    ...
    MeshResult       GenerateMesh(HAL* hal, Mesh* mesh,
    ...
    // Locks the buffers (VB/IB) of a MeshCacheItem, returning CPU addressable memory to update them. With SimpleMeshCache (consoles),
    // there is no graphics API specific "lock" that happens, it simply returns the pointers.
    ...
    virtual UInt32 GetThrashing() const { return Thrashing; }
    virtual void   ClearThrashing() { Thrashing = 0; }
...
protected:
    ...
    // Copies a GPU buffer's data from one mesh to another GPU buffer. Note that, on consoles with GPU memory addressable by the CPU,
    // this function can be executed on the CPU, instead of issuing a GPU command. Not all platforms support this ability 
    virtual void copyAndConvertGPUBuffers( Mesh* pmesh, unsigned meshIndex, PrimitiveBatch* pbatch, UPInt vertexDataOffsetInBytes, UPInt indexDataOffsetInBytes, unsigned indexStart );
    ...
    UInt32              Thrashing;          // Increases, based on LRU evictions. Only increments when AMP is enabled.

    unsigned            MeshCacheCaps;      // The capabilities of the MeshCache. Assume none, the platform-specific MeshCache::Initialize should fill these out. See
...
inline void MeshCache::copyAndConvertGPUBuffers( Mesh*, unsigned, PrimitiveBatch*, UPInt, UPInt, unsigned)
{
    SF_DEBUG_ASSERT(0, "copyAndConvertGPUBuffers not implemented on this platform, it should not be called. "
        "Cap_ConvertGPUBuffers in MeshCache::MeshCacheCaps should control calling this function.");
}
