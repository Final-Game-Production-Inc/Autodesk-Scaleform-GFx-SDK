...
// Try to preserve existing values by adding new ones to the end of each logical grouping
// e.g. 0+, 50+, 100+, 200+
enum ImageFormat
{
    Image_None              = 0,
    ...
    Image_BC7               = 53, // Compressed, D3D11 (+Durango)/GL/Orbis only.

    Image_PVRTC_RGB_4BPP    = 54, // Compressed.
    Image_PVRTC_RGBA_4BPP   = 55, // Compressed.
    Image_PVRTC_RGB_2BPP    = 56, // Compressed.
    Image_PVRTC_RGBA_2BPP   = 57, // Compressed.

    Image_ETC1_RGB_4BPP     = 58, // Compressed, compatible only with Android.
    Image_ETC1_RGBA_8BPP    = 59, // Compressed.
    Image_ETC2_RGB          = 60,
    Image_ETC2_RGBA         = 61,
    Image_ETC2_RGBA1        = 62, // Punchthrough alpha

    Image_ATCIC             = 63, // Compressed, compatible only with Android (RGB)
    Image_ATCICA            = 64, // Compressed, compatible only with Android (RGB w/explicit A).
    Image_ATCICI            = 65, // Compressed, compatible only with Android (RGB w/interpolated A).

    Image_ETC1A_RGBA_4BPP   = 66, // ETC1, with two image planes. The first is RGB, second is the alpha.
...
public:
    ...
    bool            Reserve()  { return AtomicOps<int>::CompareAndSet_Sync(&LevelCount, 0, -1); }
...
class Texture : public RefCountBase<Texture, StatRender_TextureManager_Mem>, public ListNode<Texture>, public FenceResource
{
public:
...
class DepthStencilSurface : public RefCountBase<DepthStencilSurface, StatRender_TextureManager_Mem>, public ListNode<DepthStencilSurface>, public FenceResource
{
public:    
    DepthStencilSurface(TextureManagerLocks* pmanagerLocks, const ImageSize& size, Render::MemoryManager* memMgr = 0) : 
      pManagerLocks(pmanagerLocks), State(Texture::State_InitPending), Size(size), pMemoryManager(memMgr) { }
    ...
    ImageSize                Size;
    Ptr<MemoryManager>       pMemoryManager;
};
...
public:
    ...
    // Get/Set RenderSync object. This may be NULL depending on the platform/configuration. Generally set internally in InitHAL.
    void                    SetRenderSync(Render::RenderSync* rsync) { pRenderSync = rsync; }
    Render::RenderSync*     GetRenderSync() const                    { return pRenderSync; }

    // Get MemoryManager. Console TextureManager implementations generally take a MemoryManager to their constructor if required.
    virtual Render::MemoryManager*  GetMemoryManager()               { return 0; }
...
protected:
    ...
    Ptr<RenderSync>           pRenderSync;              // RenderSync, controlling synchronization between CPU and GPU.
    Ptr<MemoryManager>        pMemoryManager;
