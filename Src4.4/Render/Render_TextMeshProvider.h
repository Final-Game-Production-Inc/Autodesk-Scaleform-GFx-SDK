...
struct VectorGlyphShape : public MeshProvider_KeySupport, public ListNode<VectorGlyphShape>
{
    ...
    virtual bool    GetData(HAL* hal, MeshBase* mesh, VertexOutput* out, unsigned meshGenFlags);
...
public:
    ...
    bool CreateMeshData(HAL* hal, const TextLayout* layout, 
    ...
    HMatrix UpdateMaskClearBounds(HAL* hal, HMatrix viewMat);
    HMatrix GetMaskClearBounds() const;

    static unsigned CalcVectorParams(HAL* hal, TextMeshLayer& layer, const TextMeshEntry& ent, 
                                     const Matrix2F& scalingMtx, 
                                     float sizeScale, const HMatrix& m, 
                                     unsigned meshGenFlags, float* keyData);
...
private:
    ...
    bool addGlyph(HAL* hal, TmpTextStorage& storage, GlyphRunData& data, unsigned glyphIndex, 
                  bool fauxBold, bool fauxItalic, bool snap, unsigned meshGenFlags);

    void createVectorGlyph(HAL* hal, unsigned layerIdx, const HMatrix& m, unsigned meshGenFlags);
    ...
    bool generateNullVectorMesh(VertexOutput* verOut);
    bool generateRectangle(HAL* hal, VertexOutput* verOut, const Matrix2F& mtx, const float* coord, 
                           UInt32 fillColor, UInt32 borderColor, unsigned meshGenFlags);

    bool generateSelection(HAL* hal, VertexOutput* verOut, const TextMeshLayer& layer, 
    ...
    bool generateUnderlines(HAL* hal, VertexOutput* verOut, const TextMeshLayer& layer, 
                            const Matrix2F& mtx, unsigned meshGenFlags);
