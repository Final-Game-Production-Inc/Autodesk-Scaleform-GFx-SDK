...
public:
    ...
    virtual bool           Update(HAL* hal, const State*) = 0;
    ...
    typedef CacheEffect* (*CreateFunc)(HAL* hal, TreeCacheNode*, const State*, CacheEffect* next);
...
public:
    ...
    bool UpdateEffects(HAL* hal, TreeCacheNode* node, unsigned changeFlags);
...
public:
    ...
    virtual bool           Update(HAL* hal, const State*);
    virtual void           ChainNext(BundleEntryRange* chain,
                                     BundleEntryRange* maskChain);
    virtual void           GetRange(BundleEntryRange* result);

    static CacheEffect*    Create(HAL* hal, TreeCacheNode*, const State*, CacheEffect* next);
};

// Base class for effects that use cacheable render targets.
class CacheableTargetEffect : public CacheEffect
{
public:
    CacheableTargetEffect(const HMatrix& m, CacheEffect* next);
...
protected:
    virtual bool canCacheAcrossTransform( bool deltaTrans, bool deltaRot, bool deltaScale ) = 0;
};

class BlendModeEffect : public CacheableTargetEffect
{
public:
    ...
    static CacheEffect*    Create(HAL* hal, TreeCacheNode*, const State*, CacheEffect* next);
protected:
    virtual bool           canCacheAcrossTransform( bool deltaTrans, bool deltaRot, bool deltaScale ) { SF_UNUSED3(deltaTrans, deltaRot, deltaScale); return true; }
};

class FilterEffect : public CacheableTargetEffect
{
public:
    FilterEffect(HAL* hal, TreeCacheNode* node, const HMatrix& m, const FilterState& state, CacheEffect* next);
    ...
    static CacheEffect*    Create(HAL* hal, TreeCacheNode*, const State*, CacheEffect* next);
protected:
    void                   updateGradientImages(HAL* hal, const FilterState& state);
    virtual bool           canCacheAcrossTransform( bool deltaTrans, bool deltaRot, bool deltaScale );
};
