...
// Uncomment this macro to send touch events to Scaleform
// IMPORTANT: make sure to add the preprocessor definition: WINVER=0x601
...
void ProcessTouchEvent(Movie *pMovie, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
            ...
            sizeof(TOUCHINPUT)))
        {
            for (UINT i = 0; i < cInputs; ++i)
            {
                ...
                {     
					...
					{
						TouchEvent evt(GFx::Event::TouchBegin, ti.dwID, (float)ptInput.x, (float)ptInput.y, (float)contact.x, (float)contact.y, primary);
						pMovie->HandleEvent(evt);
					}
                    else if (ti.dwFlags & TOUCHEVENTF_UP)
					{
						TouchEvent evt(GFx::Event::TouchEnd, ti.dwID, (float)ptInput.x, (float)ptInput.y, (float)contact.x, (float)contact.y, primary);
						pMovie->HandleEvent(evt);
					}
                    else if (ti.dwFlags & TOUCHEVENTF_MOVE)
					{
						TouchEvent evt(GFx::Event::TouchMove, ti.dwID, (float)ptInput.x, (float)ptInput.y, (float)contact.x, (float)contact.y, primary);
						pMovie->HandleEvent(evt);
					}
...
public:
    ...
    // Should return a bit mask of supported gestures
...
class TutorialThreadCommandQueue : public ThreadCommandQueue
{
public:
...
bool GFxTutorial::InitGFx()
{
    ...
    TutorialThreadCommandQueue* queue = new TutorialThreadCommandQueue;
    pCommandQueue = queue;

    // Create the renderer
    pRenderHAL = *new Render::D3D9::HAL(pCommandQueue);
	...
	pUIMovie = *pUIMovieDef->CreateInstance(true, 0, 0, pCommandQueue);
	if(!pUIMovie)
		return false;
	...
	#ifdef ENABLE_MULTITOUCH
		// Give GFx our multitouch interface so that it knows more about our touch capabilities
		pUIMovie->SetMultitouchInterface(Ptr<MultitouchInterface>(*new FxPlayerMultitouchInterface()));
	#endif ENABLE_MULTITOUCH
...
{
	#ifdef ENABLE_MULTITOUCH
		// Tell our window to send touch event messages
		HWND hWND = DXUTGetHWND();
...
void GFxTutorial::OnResetDevice(IDirect3DDevice9* pd3dDevice)
{
    ...
    if ( !pRenderHAL->IsInitialized())
        pRenderHAL->InitHAL(D3D9::HALInitParams(pd3dDevice, presentParams, D3D9::HALConfig_NoSceneCalls, Scaleform::GetCurrentThreadId()));
    else
