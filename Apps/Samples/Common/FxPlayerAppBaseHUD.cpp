...
PublicHeader:   Render
Filename    :   FxPlayerAppBaseHUD.cpp
Content     :   
Created     :   2013/02/01
Authors     :   Bart Muzzin
...
//------------------------------------------------------------------------
// FxPlayer, HUD Commands. These must come before the FxPlayerAppBaseHUD implementation,
// because they are used by it.
...
public:
    virtual void Execute(FxPlayerAppBase* app, unsigned controllerIdx, bool keyDown) const
    {
        ...
        FxPlayerAppBaseHUD* playerApp = reinterpret_cast<FxPlayerAppBaseHUD*>(app);
...
FxPlayerAppBaseHUD::FxPlayerAppBaseHUD() :
    FxPlayerAppBase()
{
...
bool FxPlayerAppBaseHUD::OnInit(Platform::ViewConfig& config)
{
...
void FxPlayerAppBaseHUD::OnShutdown()
{
...
bool FxPlayerAppBaseHUD::LoadMovie(const String& filename)
{
...
void FxPlayerAppBaseHUD::InitArgDescriptions(Platform::Args* args)
{
...
void FxPlayerAppBaseHUD::UpdateStatReports()
{
...
void FxPlayerAppBaseHUD::UpdateFpsDisplay()
{
...
void FxPlayerAppBaseHUD::UpdateViewSize() 
{
    FxPlayerAppBase::UpdateViewSize();
    UpdateHUDViewport();
}

void FxPlayerAppBaseHUD::UpdateHUDViewport()
{
    if (pHud)
    {
...
FxPlayerCommand* FxPlayerAppBaseHUD::PadKeyToCommand(PadKeyCode key)
{
...
void FxPlayerAppBaseHUD::GetExtraInfo(FxHUDExtraInfo* pinfo) 
{ 
...
void FxPlayerAppBaseHUD::GetHelpStr(String* phelpStr)
{
    phelpStr->Clear(); 
    if (!IsConsole())
    {
