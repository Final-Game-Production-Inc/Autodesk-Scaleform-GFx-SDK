...
Ptr<Render::TreeNode> VideoCharacter::CreateRenderNode(Render::Context& context) const
{
    ...
    if (pTextureImage)
    {
        ...
        if (pavm->Smoothing)
            fill.pFill->FillMode.Fill = (static_cast<unsigned int>(Render::Wrap_Clamp) | static_cast<unsigned int>(Render::Sample_Linear));
        else
            fill.pFill->FillMode.Fill = (static_cast<unsigned int>(Render::Wrap_Clamp) | static_cast<unsigned int>(Render::Sample_Point));
    ...
    shape = *context.CreateEntry<Render::TreeShape>(shapeData);
    return static_cast<Ptr<Render::TreeNode>>(shape);
}
